...
/**
 * 🌾 Smart Farm Management System — Seed Script
 * Team Antigravity | College Project
 *
 * BUG FIX: The original script used User.insertMany() which BYPASSES
 * Mongoose pre('save') hooks. This meant passwords were stored as plain
 * text and login always failed. We now use User.create() for each user
 * so the bcrypt password hashing hook runs correctly.
 */
const seed = async () => {
  try {
    ...
    // ── Admin ─────────────────────────────────────────────────────────────
    // Using User.create() so the pre('save') bcrypt hook fires correctly
    ...
    // ── Farmers ───────────────────────────────────────────────────────────
    // BUG FIX: Create each farmer with User.create() (NOT insertMany!)
    // insertMany() skips pre-save hooks → passwords stored as plain text → login fails
    const farmersRaw = [
    ...
    for (const f of farmersRaw) {
      const farmer = await User.create({ ...f, role: 'farmer' });
      farmers.push(farmer);
    }
    console.log(`✅ ${farmers.length} farmers created (passwords hashed correctly)`);
    ...
    // ── Notifications ─────────────────────────────────────────────────────
    // Send a welcome notification to the demo farmer
    await User.findByIdAndUpdate(farmers[0]._id, {
      $push: {
        notifications: {
          message: `Welcome to SmartFarm! Your land at Nashik has been approved. Documents verified. Land suitable for grape cultivation.`,
          read: false,
        }
      }
    });
    ...
    console.log('\n🌾 ─────────────────────────────────────────────────');
    console.log('   🎓 Team Antigravity — Smart Farm Management System');
